<!-- AUTO-GENERATED FILE - DO NOT EDIT -->

# AZ-900 Exam Preparation

This section contains focused materials to prepare for the **Microsoft Azure Fundamentals (AZ-900)** exam.

The goal is to combine:
- quick review 📌
- realistic practice 🧠
- exam strategy 🎯

---

## 📂 Contents

### 🧠 Exam Simulations

#### 📄 [Exam Simulation 01](exam-simulation-01.md)
- Intro-level simulation
- Covers core concepts:
  - cloud basics
  - services
  - networking
  - pricing
- Good starting point

---

#### 📄 [Exam Simulation 02](exam-simulation-02.md)
- More advanced and tricky questions
- Focus on:
  - "best choice" scenarios
  - cost optimization
  - service comparison
- Closer to real exam difficulty

---

#### 📄 [Exam Simulation 03](exam-simulation-03.md)
- Mixed-difficulty simulation
- Reinforces:
  - architecture and services
  - governance
  - identity and security
- Good for consolidating weak areas

---

#### 📄 [Exam Simulation 04](exam-simulation-04.md)
- Exam-style practice set
- Emphasis on:
  - realistic question wording
  - service distinctions
  - decision-making under pressure
- Useful before a timed review session

---

#### 📄 [Exam Simulation 05](exam-simulation-05.md)
- Final review simulation
- Best used for:
  - last-pass validation
  - confidence check
  - identifying final knowledge gaps
- Ideal shortly before the real exam

---

### ⚠️ Exam Traps

#### 📄 [Exam Traps & Patterns](exam-traps.md)
- Common pitfalls and patterns
- Helps recognize:
  - misleading wording
  - similar services (e.g. ACI vs AKS)
  - key exam keywords

👉 Highly recommended for quick review on mobile

---

## 🧭 How to Use

### Step 1 – Practice
- Start with **[Exam Simulation 01](exam-simulation-01.md)**
- Then move to **[Exam Simulation 02](exam-simulation-02.md)**
- Continue with **[Exam Simulation 03](exam-simulation-03.md)**
- Use **[Exam Simulation 04](exam-simulation-04.md)** as timed practice
- Finish with **[Exam Simulation 05](exam-simulation-05.md)** for final review

---

### Step 2 – Analyze mistakes
- Identify weak areas
- Revisit:
  - cheat sheet
  - lab summaries

---

### Step 3 – Memorize patterns
- Read **[Exam Traps](exam-traps.md)**
- Focus on keywords and typical questions

---

## 🎯 Goal

You are ready for the exam if:

- You consistently score **80%+**
- You understand *why* answers are correct
- You can distinguish similar services quickly

---

## 💡 Tip

In the exam:
- Read questions carefully
- Look for keywords like:
  - *best*
  - *least effort*
  - *most cost-effective*

👉 These often determine the correct answer.

<!-- AUTO-GENERATED FILE - DO NOT EDIT -->

## Markdown Files

- [Exam Simulation 01](exam-simulation-01.md)
- [Exam Simulation 02](exam-simulation-02.md)
- [Exam Simulation 03](exam-simulation-03.md)
- [Exam Simulation 04](exam-simulation-04.md)
- [Exam Simulation 05](exam-simulation-05.md)
- [Exam Simulation 06](exam-simulation-06.md)
- [Exam Traps](exam-traps.md)
